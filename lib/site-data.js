...
function mapDbContentToContent(row) {
  ...
  const fallbackBody = [
    row.contact_paragraph_two,
    row.contact_paragraph_three,
    row.contact_paragraph_four,
  ]
    .filter(Boolean)
    .join("\n\n");
  ...
  return {
    ...
    contactBody: fallbackBody || defaultSiteData.content.contactBody,
  };
}
...
export async function saveSiteData(siteData) {
  ...
  const contentPayload = mapSiteDataToDbContent(normalized.content);
