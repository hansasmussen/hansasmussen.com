...
function normalizeCarouselOrder(value, items = []) {
...
function buildVideoPreviewConfig(item, rawVideoPreviewCatalog) {
  ...
  return {
    ...
    youtubeUrl: String(config?.youtubeUrl || item.youtubeUrl || "").trim() || null,
  };
}
...
function buildVideoPreviewCatalog(items) {
  return items.reduce((catalog, item) => {
    if (
      ...
      (!item.videoPreview?.src && !item.videoPreview?.youtubeUrl)
    ) {
      return catalog;
    }
    ...
    catalog[item.id] = {
      ...
      youtubeUrl: String(item.videoPreview.youtubeUrl || "").trim(),
    };
...
export function normalizeSiteData(rawSiteData) {
  ...
  const normalizedPortfolioItems = normalizePortfolioItems(
  ...
  return normalizeTypographyTree({
    ...
    content: {
      ...
      ...(rawSiteData.content || {}),
      carouselOrder: normalizeCarouselOrder(rawSiteData.content?.carouselOrder || [], normalizedPortfolioItems),
...
function mapDbContentToContent(row) {
  ...
  return {
    ...
    carouselOrder: Array.isArray(printContent.carouselOrder) ? printContent.carouselOrder : [],
...
function mapSiteDataToDbContent(content) {
  return {
    ...
    contact_paragraph_four: JSON.stringify({
      ...
      carouselOrder: Array.isArray(content.carouselOrder) ? content.carouselOrder : [],
...
export function getOrderedFeaturedItems(siteData) {
  const featuredItems = (siteData.portfolioItems || []).filter((item) => item.featured);
  const itemById = new Map(featuredItems.map((item) => [String(item.id), item]));
  const orderedIds = normalizeCarouselOrder(siteData.content?.carouselOrder || [], featuredItems);
  return orderedIds.map((id) => itemById.get(id)).filter(Boolean);
}

export function getFeaturedSlides(siteData) {
  return getOrderedFeaturedItems(siteData).map((item) => ({
